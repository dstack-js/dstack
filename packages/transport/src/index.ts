...
import type {
  PeerDiscovery,
  PeerDiscoveryEvents
} from '@libp2p/interfaces/peer-discovery'
import type {
  WebRTCStarSocket,
  HandshakeSignal
...
const log = logger('dstack:transport')

// eslint-disable-next-line @typescript-eslint/no-empty-function
const noop = () => {}

class WebRTCStarDiscovery
  ...
  override dispatchEvent(event: CustomEvent) {
    if (!this.isStarted()) {
      return false
    }
...
export interface WebRTCStarListenerOptions
  ...
  async _connect(ma: Multiaddr, options: WebRTCStarDialOptions) {
    ...
    return await new Promise<WebRTCInitiator>((resolve, reject) => {
      ...
      if (sio?.socket == null) {
        return reject(
          errcode(
            new Error('unknown signal server to use'),
