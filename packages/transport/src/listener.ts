...
import type {
  ...
  MultiaddrConnection,
  ListenerEvents
} from '@libp2p/interfaces/transport'
import type {
  WebRTCStar,
  WebRTCStarListenerOptions,
  SignalServer,
...
import type {
  WebRTCStarSocket,
  HandshakeSignal
} from '@libp2p/webrtc-star-protocol'
import { EventEmitter, CustomEvent } from '@libp2p/interfaces'

const log = logger('dstack:transport:listener')
...
class SigServer
  extends EventEmitter<SignalServerServerEvents>
  implements SignalServer {
  ...
  constructor(
    ...
    this.socket = connect(signallingUrl, {
      ...sioOptions,
      extraHeaders: { 'X-DStack-Namespace': namespace }
    })
    this.connections = []
          ...
          this.upgrader
            ...
            .catch((err) => {
              ...
              maConn.close().catch((err) => {
                log.error(
                  'inbound connection failed to close after failing to upgrade',
                  err
                )
              })
            })
  ...
  handleWsHandshake(offer: HandshakeSignal) {
    log(
      'incoming handshake. signal type "%s" is answer %s',
      offer.signal.type,
      offer.answer
    )
    ...
    if (channel == null) {
      if (offer.signal.type !== 'offer') {
        log(
          'handshake is not an offer and channel does not exist, buffering until we receive an offer'
        )
        return
      }

      log('creating new channel to handle offer handshake')
      channel = this._createChannel(
        offer.intentId,
        offer.srcMultiaddr,
        offer.dstMultiaddr
      )
  ...
  async close() {
    ...
    await Promise.all([
      ...
      ...Array.from(this.channels.values()).map(
        async (channel) => await channel.close()
      )
    ])
...
class WebRTCListener extends EventEmitter<ListenerEvents> implements Listener {
  ...
  constructor(
    ...
    this.namespace = transport.namespace
  }
  ...
  async listen(ma: Multiaddr) {
    ...
    if (this.listeningAddr != null) {
      throw errCode(
        new Error('listener already in use'),
    ...
    const server: SignalServer = new SigServer(
      this.signallingUrl,
      signallingAddr,
      this.upgrader,
      this.handler,
